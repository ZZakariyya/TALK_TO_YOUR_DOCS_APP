...
def process_file(file: AskFileResponse):
    ...
    try:
        ...
        if file.type == "text/plain":
            ...
            # Loading CSV or Excel file content into a DataFrame
            df = pd.read_csv(temp_file_path) if file.type == "text/csv" else pd.read_excel(temp_file_path)
            # Converting the DataFrame to text (you might want to format this differently)
            text_content = df.to_string(index=False)
            # Create a text loader with the textual representation of the DataFrame
